export class EmailService {
    static async sendShareLink(email: string, link: string, senderName: string, fileName: string) {
        console.log('[DEBUG] Sending Share Link via Brevo API');

        const apiKey = process.env.BREVO_API_KEY;
        const senderEmail = process.env.SMTP_USER || 'sriharshatejasakalabathula@gmail.com';

        if (!apiKey) {
            console.error('[EMAIL SERVICE] Missing BREVO_API_KEY!');
            return false;
        }
        ...
        try {
            ...
            if (!response.ok) {
                ...
                console.error('[EMAIL SERVICE] Brevo Error Data:', errorData);
                return false;
            }

            console.log(`[EMAIL SERVICE] Email sent via Brevo API successfully.`);
            return true;
        } catch (error: any) {
            console.error('[EMAIL SERVICE] Error sending email via Brevo:', error);
            return false;
        }
    }

    static async sendOtpEmail(email: string, otp: number, userName: string) {
        console.log('[DEBUG] Sending OTP via Brevo API');
        ...
        try {
            ...
            console.log(`[EMAIL SERVICE] OTP Email sent via Brevo API successfully.`);
            return true;
        } catch (error: any) {
            console.error('[EMAIL SERVICE] Error sending OTP email via Brevo:', error);
            return false;
        }
    }
}
